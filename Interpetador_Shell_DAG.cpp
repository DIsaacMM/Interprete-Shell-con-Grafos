...
void DAG_create()
{
    ...
    while (true)
    {
        ...
        string t = trim(input);
        if (t == "DAG_execute")                               // Checar si el comando que se puso es DAG_execute()
        {
...
void DAG_execute(std::vector<std::string>& dag_commands)
{
    ...
    for (const string& dag_cmd_raw : dag_commands) // For recorre cada comando en el vector de comandos
    {
        string dag_cmd = trim(dag_cmd_raw);
        if (dag_cmd.empty()) continue;
        ...
        size_t second_dash = string::npos;
        if (first_dash != string::npos) second_dash = dag_cmd.find(" - ", first_dash + 3); // Busca el segundo " - " empezando despues del primero
        ...
        else
        {
            ...
            // Parsear dependencias separadas por comas, trimming y evitando vacíos
            size_t start = 0;
            while (start < deps_str.size())
            {
                size_t end = deps_str.find(',', start);
                string token;
                if (end == string::npos) token = deps_str.substr(start);
                else token = deps_str.substr(start, end - start);
        ...
        {
            cout << "Error: Nodo sin nombre/archivo de salida en: " << dag_cmd << endl;
            continue;
        }
    ...
    for (const auto& pair : nodes) // Itera sobre cada par clave-valor en el mapa de nodos
    {
        ...
        for (const string& dep : node->deps) // Itera sobre cada dependencia del nodo actual
        {
            ...
            if (dep.empty() || nodes.find(dep) == nodes.end())
            {
    ...
    // 3. Inicializar cola de tareas listas (sin dependencias)
    {
        ...
        remaining_tasks = static_cast<int>(nodes.size());  // Inicializa el contador de tareas pendientes con el total de nodos
    }
                ...
                while (true)
                {
                    ...
                    if (task)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                lock_guard<mutex> log_lock(log_mtx);
                                perror("fork failed for task execution");
                            }
                            result = -1;
                        }
                        ...
                        // Notificar a los hijos que esta tarea terminó o reintentar si aplica
                        {
...
{
    if (cmd.empty()) return;
    // Si hay espacio en el historial
    ...
    {
        // Desplazar todos hacia la izquierda para hacer espacio (FIFO)
...
const char* get_history_up()
{
    ...
    // Si estamos en 0, permanecer en 0 y retornar el primer
    current_index = 0;
...
void read_input(char* buffer)
{
    ...
    buffer[0] = '\0'; // Inicializar buffer vacío
                ...
                if (seq[0] == '[')
                {
                    if (seq[1] == 'A')
                    {
                        ...
                        if (prevCmd)
                        {
                            ...
                            len = static_cast<int>(snprintf(buffer, MAX_INPUT, "%s", prevCmd));
                    ...
                    else if (seq[1] == 'B')
                    {
                        ...
                        if (nextCmd && strlen(nextCmd) > 0)
                        {
                            ...
                            len = static_cast<int>(snprintf(buffer, MAX_INPUT, "%s", nextCmd));
                            std::cout << buffer; // Muestra el comando
                            std::cout.flush();
                        }
...
int main()
{
    ...
    // Comando "DAG_create()"
    command["DAG_create"] = [](char* args[]) -> bool
        {
            DAG_create();
            return true;
        };
